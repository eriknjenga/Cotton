...
class Truck_Transactions extends MY_Controller {
	...
	public function download() {
		...
		$sql = "select str_to_date(transaction_date,'%d/%m/%Y') as transaction_date,vehicle_number,depot_code,depot_name,distance, buying_center_code,net_weight,ticket_number from weighbridge w left join depot d on w.buying_center_code = d.depot_code where vehicle_number = '" . $truck . "' and weighing_type = '2' and str_to_date(transaction_date,'%d/%m/%Y') between str_to_date('" . $start_date . "','%m/%d/%Y') and str_to_date('" . $end_date . "','%m/%d/%Y') order by str_to_date(transaction_date,'%d/%m/%Y') desc";
		$query = $this -> db -> query($sql);
		foreach ($query->result_array() as $depot_data) {
			$data_buffer .= "<tr><td>" . $depot_data['transaction_date'] . "</td><td>" . $depot_data['depot_name'] . "</td><td>" . $depot_data['distance'] . "</td><td>" . $depot_data['ticket_number'] . "</td><td>" . number_format($depot_data['net_weight']+0) . "</td></tr>";
			$total_weight += $depot_data['net_weight'];
			$total_distance += $depot_data['distance'];
		}
		$data_buffer .= "<tr></tr><tr><td>Totals: </td><td>-</td><td>" . number_format($total_distance+0) . "</td><td>-</td><td>" . number_format($total_weight+0) . "</td></tr>";
		$data_buffer .= "</table>";
		$log = new System_Log();
		$log -> Log_Type = "4";
	...
	public function echoTitles() {
		return "<tr><th>Transaction Date</th><th>Buying Center</th><th>Distance (Kms)</th><th>Ticket Number</th><th>Net Weight (Kgs.)</th></tr>";
	}
