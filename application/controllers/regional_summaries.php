...
class Regional_Summaries extends MY_Controller {
	...
	public function download() {
		...
		$data_buffer .= "</table>";
		$data_buffer .= "<h3>Summaries</h3><table class='data-table'><tr><th>Zone</th><th>Total Cash Received</th><th>Total Purchases (Tsh.)</th><th>Total Purchases (Kgs.)</th><th>Total Dispatch (Kgs.)</th><th>Avg. Per KG.</th><th>Total Cash Balance</th><th>Total Product Balance</th><th>Avg. Distance</th></tr>";
		foreach ($regions as $region) {
			$avg_per_kg = 0;
		...
		$data_buffer .= "<tr></tr><tr><td><b>Grand Totals</b></td><td class='amount'>" . number_format($total_cash_received) . "</td><td class='amount'>" . number_format($total_purchases_value) . "</td><td class='amount'>" . number_format($total_purchases_kg) . "</td><td class='amount'>" . number_format($total_dispatch) . "</td><td class='amount'>" . number_format($avg_per_kg) . "</td><td class='amount'>" . number_format(($total_cash_received - $total_cash_paid - $total_purchases_value)) . "</td><td class='amount'>" . number_format(($total_purchases_kg - $total_dispatch)) . "</td><td class='amount'>" . $grand_avg_distance . "</td></tr>";
		$data_buffer .= "</table>";
		//echo $data_buffer;
		$log = new System_Log();
		$log -> Log_Type = "4";
	...
	public function downloadExcel($regions) {
		...
		foreach ($regions as $region) {
			...
			foreach ($region_depots->result_array() as $depot) {
				if ($depot['deleted'] != "1") {
					...
					$depot_data = $query -> row_array();
					$avg_distance = ($depot_data['purchases_kg'] * $depot['distance']);
					$data_buffer .= $depot_data['depot_name'] . "\t" . $depot_data['depot_code'] . "\t" . $status[$depot['deleted']] . "\t" . $depot['cash_disbursement_route'] . "\t" . $depot_data['last_transaction_date'] . "\t" . $depot_data['cash_received'] . "\t" . $depot_data['purchases_value'] . "\t" . $depot_data['purchases_kg'] . "\t" . $depot_data['dispatch'] . "\t" . $depot_data['avg_per_kg'] . "\t" . $depot_data['cash_balance'] . "\t" . $depot_data['product_balance'] . "\t" . $depot_data['last_price'] . "\t\n";
					$region_summaries[$region -> id]['total_cash_received'] += $depot_data['cash_received'];
					$region_summaries[$region -> id]['total_purchases_value'] += $depot_data['purchases_value'];
					$region_summaries[$region -> id]['total_purchases_kg'] += $depot_data['purchases_kg'];
					$region_summaries[$region -> id]['total_dispatch'] += $depot_data['dispatch'];
		...
		$data_buffer .= "Summaries\nZone\tTotal Cash Received\tTotal Purchases (Tsh.)\tTotal Purchases (Kgs.)\tTotal Dispatch (Kgs.)\tAvg. Per KG.\tTotal Cash Balance\tTotal Product Balance\tTotal Avg. Distance\t\n";
		foreach ($regions as $region) {
			$avg_per_kg = 0;
			$total_cash_received += $region_summaries[$region -> id]['total_cash_received'];
			$total_purchases_value += $region_summaries[$region -> id]['total_purchases_value'];
		...
		$data_buffer .= "\nGrand Totals\t" . $total_cash_received . "\t" . $total_purchases_value . "\t" . $total_purchases_kg . "\t" . $total_dispatch . "\t" . $avg_per_kg . "\t" . ($total_cash_received - $total_cash_paid - $total_purchases_value) . "\t" . ($total_purchases_kg - $total_dispatch) . "\t" . $grand_avg_distance . "\t\n";
	...
	public function echoTitles() {
		return "<tr><th>Buying Center</th><th>BC Code</th><th>Status</th><th>Cashier Route</th><th>Last Purchase Date</th><th>Cash Received</th><th>Purchases (Tsh.)</th><th>Purchases (Kgs.)</th><th>Dispatch (Kgs.)</th><th>Avg. Per KG.</th><th>Cash Balance</th><th>Product Balance</th><th>Last Price</th></tr>";
	}

	public function echoExcelTitles() {
		return "Buying Center\tBC Code\tStatus\tCashier Route\tLast Purchase Date\tCash Received\tPurchases (Tsh.)\tPurchases (Kgs.)\tDispatch (Kgs.)\tAvg. Per KG.\tCash Balance\tProduct Balance\tLast Price\t\n";
	}
