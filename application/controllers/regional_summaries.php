...
class Regional_Summaries extends MY_Controller {
	...
	public function download() {
		...
		foreach ($regions as $region) {
			...
			foreach ($region_depots->result_array() as $depot_data) {
				...
				$region_summaries[$region -> id]['total_loan_recoveries'] += $depot_data['recoveries'];
				$region_summaries[$region -> id]['total_avg_distance'] += $avg_distance;
			}
		}
		$data_buffer .= "</table>";
		$data_buffer .= "<h3>Summaries</h3><table class='data-table'><tr><th>Zone</th><th>Total Cash Received</th><th>Total Cash Returned</th><th>Total Loan Recoveries</th><th>Total Purchases (Tsh.)</th><th>Total Purchases (Kgs.)</th><th>Total Dispatch (Kgs.)</th><th>Avg. Price/KG.</th><th>Total Cash Balance</th><th>Total Stock Balance</th><th>Avg. Distance</th></tr>";
		foreach ($regions as $region) {
			$avg_per_kg = 0;
			...
			$total_loan_recoveries += $region_summaries[$region -> id]['total_loan_recoveries'];
			$zone_avg_distance = 0;
			...
			$data_buffer .= "<tr><td>" . $region -> Region_Name . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_cash_received']) . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_cash_returned']) . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_loan_recoveries']) . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_purchases_value']) . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_purchases_kg']) . "</td><td class='amount'>" . number_format($region_summaries[$region -> id]['total_dispatch']) . "</td><td class='amount'>" . number_format($avg_per_kg) . "</td><td class='amount'>" . number_format(($region_summaries[$region -> id]['total_cash_received'] - $region_summaries[$region -> id]['total_purchases_value']- $region_summaries[$region -> id]['total_cash_returned']+ $region_summaries[$region -> id]['total_loan_recoveries'])) . "</td><td class='amount'>" . number_format(($region_summaries[$region -> id]['total_purchases_kg'] - $region_summaries[$region -> id]['total_dispatch'])) . "</td><td class='amount'>" . $zone_avg_distance . "</td></tr>";
		}
		$avg_per_kg = 0;
		...
		$data_buffer .= "<tr></tr><tr><td><b>Grand Totals</b></td><td class='amount'>" . number_format($total_cash_received) . "</td><td class='amount'>" . number_format($total_cash_returned) . "</td><td class='amount'>" . number_format($total_loan_recoveries) . "</td><td class='amount'>" . number_format($total_purchases_value) . "</td><td class='amount'>" . number_format($total_purchases_kg) . "</td><td class='amount'>" . number_format($total_dispatch) . "</td><td class='amount'>" . number_format($avg_per_kg) . "</td><td class='amount'>" . number_format(($total_cash_received - $total_cash_returned - $total_purchases_value+$total_loan_recoveries)) . "</td><td class='amount'>" . number_format(($total_purchases_kg - $total_dispatch)) . "</td><td class='amount'>" . $grand_avg_distance . "</td></tr>";
		$data_buffer .= "</table>";
		//echo $data_buffer;
		$log = new System_Log();
		$log -> Log_Type = "4";
	...
	public function downloadExcel($regions) {
		...
		$data_buffer .= "Summaries\nZone\tTotal Cash Received\tTotal Cash Returned\tTotal Loan Recovered\tTotal Purchases (Tsh.)\tTotal Purchases (Kgs.)\tTotal Dispatch (Kgs.)\tAvg. Per KG.\tTotal Cash Balance\tTotal Product Balance\tTotal Avg. Distance\t\n";
		foreach ($regions as $region) {
			$avg_per_kg = 0;
			$total_cash_received += $region_summaries[$region -> id]['total_cash_received'];
			$total_cash_returned += $region_summaries[$region -> id]['total_cash_returned'];
		...
		$data_buffer .= "\nGrand Totals\t" . $total_cash_received . "\t" . $total_cash_returned . "\t" . $total_loan_recoveries . "\t" . $total_purchases_value . "\t" . $total_purchases_kg . "\t" . $total_dispatch . "\t" . $avg_per_kg . "\t" . ($total_cash_received - $total_cash_returned + $total_loan_recoveries - $total_purchases_value) . "\t" . ($total_purchases_kg - $total_dispatch) . "\t" . $grand_avg_distance . "\t\n";
	...
	public function echoTitles() {
		return "<thead><tr><th>BC Name</th><th>BC Code</th><th>Status</th><th>Cashier Route</th><th>Last Purchase Date</th><th>Cash Received</th><th>Cash Returned</th><th>Loan Recoveries</th><th>Purchases (Tsh.)</th><th>Purchases (Kgs.)</th><th>Dispatch (Kgs.)</th><th>Avg. Price/KG.</th><th>Cash Balance</th><th>Stock Balance</th><th>Last Price</th></tr></thead>";
	}

	public function echoExcelTitles() {
		return "Buying Center\tBC Code\tStatus\tCashier Route\tLast Purchase Date\tCash Received\tCash Returned\tLoan Recoveries\tPurchases (Tsh.)\tPurchases (Kgs.)\tDispatch (Kgs.)\tAvg. Price/KG.\tCash Balance\tStock Balance\tLast Price\t\n";
	}
