...
class Dispatch_Recommendation extends MY_Controller {
	...
	public function download() {
		...
		foreach ($query->result_array() as $depot_data) {
			...
			$data_buffer .= "<tr><td>" . $depot_data['depot_name'] . "</td><td class='center'>" . $depot_data['depot_code'] . "</td><td class='center'>" . $depot_data['purchase_route'] . "</td><td class='center'>" . (empty($depot_data['distance']) ? '-' : $depot_data['distance']) . "</td><td class='center'>" . (empty($depot_data['depot_capacity']) ? '-' : $depot_data['depot_capacity']) . "</td><td class='amount'>" . (empty($depot_data['product_balance']) ? '-' : number_format($depot_data['product_balance'] + 0)) . "</td><td class='center'>" . (empty($depot_data['ratio']) ? '-' : number_format($depot_data['ratio'],2)) . "</td><td class='center'>" . (empty($depot_data['formatted_dispatch_date']) ? '-' : $depot_data['formatted_dispatch_date']) . "</td><td class='center'>" . (empty($depot_data['date_closed']) ? '-' : $depot_data['date_closed'] + 0) . "</td><td class='center'>" . $days_pending . "</td><td class='center'>" . (empty($depot_data['number_plate']) ? '-' : $depot_data['number_plate']) . "</td><td class='center'>" . (empty($depot_data['date']) ? '-' : $depot_data['date']) . "</td></tr>";
	...
	public function downloadExcel($date, $maximum_stock, $capacity_percentage) {
		...
		foreach ($query->result_array() as $depot_data) {
			...
			$data_buffer .= $depot_data['depot_name'] . "\t" . $depot_data['depot_code'] . "\t" . $depot_data['purchase_route'] . "\t" . $depot_data['distance'] . "\t" . $depot_data['depot_capacity'] . "\t" . $depot_data['product_balance'] . "\t" . number_format(($depot_data['ratio']+0),2)."\t". $depot_data['formatted_dispatch_date'] . "\t" . $depot_data['date_closed'] . "\t" . $days_pending . "\t" . $depot_data['number_plate'] . "\t" . $depot_data['date'] . "\t\n";
			$total_balance += $depot_data['product_balance'];
		}
	...
	public function echoTitles() {
		return "<thead><tr><th>Buying Center</th><th>Center Code</th><th>Route</th><th>Distance</th><th>Capacity(Tonnes)</th><th>Product Balance</th><th>Ratio</th><th>Last Dispatch Date</th><th>Closed On</th><th>Days Pending</th><th>Truck Sent</th><th>Date Sent</th></tr></thead>";
	}

	public function echoExcelTitles() {
		return "Buying Center\tCenter Code\tRoute\tDistance\tCapacity (Tonnes)\tProduct Balance\tRatio\tLast Dispatch Date\tClosed On\tDays Pending\tTruck Sent\tDate Sent\t\n";
	}
	...
	function generatePDF($data, $date, $capacity, $percentage) {
		...
		$this -> mpdf -> SetFooter('Generated on: {DATE d/m/Y}|{PAGENO}|Dispatch Recommendation Report');
