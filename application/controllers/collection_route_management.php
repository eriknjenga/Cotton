<?php
class Collection_Route_Management extends MY_Controller {
	...
	public function listing($offset = 0) {
		$items_per_page = 20;
		$number_of_routes = Collection_Route::getTotalRoutes();
		$routes = Collection_Route::getPagedRoutes($offset, $items_per_page);
		if ($number_of_routes > $items_per_page) {
			$config['base_url'] = base_url() . "collection_route_management/listing/";
		...
		$data['routes'] = $routes;
		$data['title'] = "Routes";
		$data['content_view'] = "list_collection_routes_v";
	...
	public function new_route($data = null) {
		...
		$data['content_view'] = "add_collection_route_v";
		$data['quick_link'] = "new_collection_route";
	...
	public function edit_route($id) {
		$route = Collection_Route::getRoute($id);
		$data['route'] = $route;
		$this -> new_route($data);
	}
	...
	public function save() {
		...
		if ($valid) {
			...
			if (strlen($editing) > 0) {
				$route = Collection_Route::getRoute($editing);
			} else {
				$route = new Collection_Route();
			}
			$route -> Route_Code = $this -> input -> post("route_code");
			$route -> Route_Name = $this -> input -> post("route_name");
			$route -> save();
			redirect("collection_route_management/listing");
		} else {
			$this -> new_route();
		}
	}
	...
	public function base_params($data) {
		$data['sub_link'] = "collection_route_management";
