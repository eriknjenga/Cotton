...
class Agent_Input_Issue extends Doctrine_Record {
	...
	public function getTotalIssues() {
		$query = Doctrine_Query::create() -> select("count(*) as Total_Issues") -> from("Agent_Input_Issue");
		$total = $query -> execute();
		return $total[0]['Total_Issues'];
	}

	public function getPagedIssues($offset, $items) {
		$query = Doctrine_Query::create() -> select("*") -> from("Agent_Input_Issue") -> offset($offset) -> limit($items);
